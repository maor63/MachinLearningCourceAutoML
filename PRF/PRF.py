...
class DecisionTreeClassifier:
    ...
    def fit(self, X, pX, py):
        ...
        self.tree_ = tree.fit_tree(X, pX, py_gini, py_leafs, pnode, depth, is_max, self.max_depth, self.max_features,
                                   self.feature_importances_, self.n_samples_, self.keep_proba, self.unsupervised,
                                   self.new_syn_data_frac, self.min_py_sum_leaf)
    ...
    def predict_proba(self, X, dX, return_leafs=False):
        ...
        result = tree.predict_all(self.node_tree_results, self.node_feature_idx, self.node_feature_th,
                                  self.node_true_branch, self.node_false_branch, self.node_p_right, X, dX, keep_proba,
                                  return_leafs)
...
class RandomForestClassifier(BaseEstimator):
    ...
    def fit(self, X, y=None, dX=None, py=None):
        """
